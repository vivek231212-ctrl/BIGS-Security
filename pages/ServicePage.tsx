  ...
  return (
    <div className="pt-24">
      ...
      <section className="relative h-[50vh] flex items-center overflow-hidden">
        ...
        <div className="container mx-auto px-[30px] relative z-10 text-white">
          <div className="flex items-center gap-2 mb-6 text-[#D30000] font-black uppercase text-xs tracking-widest">
             <Link to="/" className="hover:text-white">Home</Link> <span>/</span> <span>Services</span>
          </div>
          <h2 className="text-4xl md:text-6xl font-black mb-4 uppercase tracking-tighter leading-none">{service.title}</h2>
          <p className="text-lg sm:text-xl text-white/70 max-w-2xl font-medium">{service.tagline}</p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-[30px]">
          <div className="grid lg:grid-cols-3 gap-16">
            <div className="lg:col-span-2">
              ...
              <h2 className="text-3xl font-black text-black mb-8 uppercase tracking-tight">Service Overview</h2>
              <p className="text-black/60 text-lg leading-relaxed mb-10 font-medium">{service.scope}</p>
              
              <div className="grid md:grid-cols-2 gap-8 mb-16">
                {service.features.map((f: string, i: number) => (
                  <div key={i} className="flex gap-4 p-8 bg-[#F5F5F5] border-l-4 border-[#D30000] items-center">
                    <CheckCircle className="text-[#D30000] flex-shrink-0 w-6 h-6" />
                    <span className="font-black text-black uppercase tracking-tight text-sm">{f}</span>
                  </div>
                ))}
              </div>

              <h2 className="text-3xl font-black text-black mb-8 uppercase tracking-tight">Technology & Tools</h2>
              <div className="p-10 bg-black text-white rounded-md border-b-8 border-[#D30000] mb-16">
                <Zap className="w-12 h-12 text-[#D30000] mb-6" />
                <p className="text-xl leading-relaxed font-medium">{service.tech}</p>
              </div>

              <h2 className="text-3xl font-black text-black mb-8 uppercase tracking-tight">Core Value</h2>
              <div className="space-y-6">
                {service.benefits.map((b: string, i: number) => (
                  <div key={i} className="flex items-center gap-4 text-black font-bold border-b border-slate-100 pb-4">
                    ...
                    <span className="text-lg uppercase tracking-tight">{b}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-10">
              <div className="bg-[#F5F5F5] p-10 border border-slate-200">
                <h3 className="text-sm font-black text-[#D30000] mb-8 uppercase tracking-[0.3em]">Our Services</h3>
                <div className="space-y-2">
                  {Object.entries(serviceDetails).map(([id, s]) => (
                    <Link 
                      key={id} 
                      to={`/services/${id}`}
                      className={`flex items-center justify-between p-4 font-black uppercase text-[10px] tracking-widest transition-all ${serviceId === id ? 'bg-[#D30000] text-white' : 'bg-white hover:bg-black hover:text-white text-black shadow-sm'}`}
                    >
              ...
              <div className="bg-[#D30000] p-12 text-white text-center">
                <h3 className="text-2xl font-black mb-4 uppercase tracking-tighter">Request Audit</h3>
                <p className="mb-10 text-white/80 font-medium">Get a specialized consultation for your specific security needs.</p>
                <Link to="/get-quote" className="block w-full bg-black text-white py-5 font-black uppercase text-xs tracking-widest hover:bg-white hover:text-black transition-all shadow-2xl">
                  Contact Specialist
                </Link>
              </div>
            </div>
